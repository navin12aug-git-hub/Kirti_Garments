...
export default function Header({ onMobileMenu }) {
  ...
  return (
    <>
      <header className={`sticky top-0 z-[60] bg-white transition-shadow duration-300 ${scrolled ? 'shadow-md' : 'shadow-sm'}`}>
        <AnnouncementBar />
        <div className="border-b border-neutral-200">
          <div className="container-custom">
            <div className="flex items-center justify-between h-16 md:h-20">
              ...
              {/* Center: nav */}
              <nav className="hidden lg:flex items-center gap-6" onMouseLeave={() => setHovered(null)}>
                {navItems.map((item) => (
                  <div
                    key={item.label}
